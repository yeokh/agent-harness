...
=================================
Flask web UI that drives the Amazon Strands-based file-processing agent.
All agent logic lives in strands_agent.py; this file handles only HTTP
...
Endpoints:
  ...
  PUT  /api/file/inbox/<path>     → save edited content to an inbox file  ★ NEW
...
# ────────────────────────────────────
# The guardrail check runs inside _agent_thread() and uses add_log() to
# report its verdict.  AgentState.finish() marks the run as "error" if the
# guardrail blocks — the browser will show the reason in the log terminal.
...
class AgentState:
    ...
    def __init__(self) -> None:
        ...
        self._model: str             = strands_agent.default_model()
...
#       ...
#
# HINT: Use strands.Agent with a fast model (e.g. claude-haiku-4-5) and a
# fixed system prompt that looks for prompt injection, path traversal, and
# secret exfiltration.  Return SAFE/UNSAFE based on the model's response.
#
...
# WORKSHOP (Exercise 01 — Explore)
# ─────────────────────────────────
# _agent_thread() is the bridge between Flask and strands_agent.py.
# It runs as a daemon thread so it does not block HTTP request handling.
#
# Lifecycle:
#   1. state.start()                    → sets status="running", clears old logs
#   2. strands_agent.run_agent()        → blocks until the Strands agent finishes
...
# WORKSHOP (Exercise 04 — Guardrail)
# ────────────────────────────────────
# Add your guardrail call here, between state.start() and
# strands_agent.run_agent().  If the guardrail returns is_safe=False,
# call state.finish(reason) and return early — the agent never runs.
# See the PLACEHOLDER comment below.
#
# WORKSHOP (Exercise 06 — Multi-Agent Pipeline)
# ──────────────────────────────────────────────
# To add a pipeline, create _pipeline_thread() below this function and add a
# new /api/agent/pipeline route.  See the PLACEHOLDER below and
# workshop/advanced/06_multi_agent.md.

def _agent_thread() -> None:
    """Background thread: runs the Strands agent and streams output to AgentState."""
    state.start()
    ...
    try:
        with log_path.open("w", encoding="utf-8") as log_fh:
            ...
            # Hand off to strands_agent.py — this call blocks until the
            # Strands agent finishes its agentic loop.
...
#   2. Stage 1: call strands_agent.run_agent() with a fast/cheap model into a
#               temporary directory (use tempfile.TemporaryDirectory)
#   3. Stage 2: call strands_agent.run_agent() with the selected model, reading
...
#
# NEW FEATURE: This endpoint lets the browser save edited content back to an
# inbox file.  Only inbox files are editable — outbox is read-only from the UI.
#
# The browser sends:  PUT /api/file/inbox/<filename>
#                     Content-Type: application/json
#                     {"content": "<new file content>"}
#
# Security: _safe_path() rejects any path-traversal attempt before writing.
...
# The list is filtered dynamically: if you add a new API key and restart,
# the new models appear automatically — no UI changes needed.

@app.route("/api/model", methods=["GET"])
def api_get_model():
    available = strands_agent.get_available_models()
    return jsonify({
        "model":   state.model,
        "models":  available,
...
def api_set_model():
    ...
    allowed   = [m["id"] for m in strands_agent.get_available_models()]
...
#
# The *offset* parameter prevents re-sending lines if the browser reconnects.
