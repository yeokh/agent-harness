#!/usr/bin/env python3
"""
Smolagents Agent — Web Application
====================================
Flask web UI that drives the HuggingFace Smolagents file-processing agent.
All agent logic lives in smolagent_agent.py; this file handles only HTTP
...
#
# This class is IDENTICAL to the ADK version — the web layer is provider-agnostic.
# Only the import (smolagent_agent vs adk_agent) differs between the two projects.
#
# WORKSHOP (Exercise 04 — Guardrail)
# ────────────────────────────────────
# The guardrail check runs inside _agent_thread() and uses add_log() to report
# its verdict.  AgentState.finish() marks the run as "error" if blocked.
...
class AgentState:
    ...
    def __init__(self) -> None:
        ...
        self._model: str             = smolagent_agent.default_model()
...
# WORKSHOP (Exercise 01 — Explore)
# ─────────────────────────────────
# _agent_thread() is the bridge between Flask and smolagent_agent.py.
# It runs as a daemon thread so it does not block HTTP request handling.
#
# Lifecycle:
#   1. state.start()                   → sets status="running", clears old logs
#   2. smolagent_agent.run_agent()     → blocks until the smolagents agent finishes
#   3. state.finish()                  → sets status="completed" or "error"
#
# COMPARISON with ADK:
#   This function is structurally IDENTICAL to the ADK version.
#   The only difference is the import: smolagent_agent vs adk_agent.
#   This is intentional — the web layer does not need to know which
#   agent framework is running underneath.
#
# WORKSHOP (Exercise 04 — Guardrail)
# ────────────────────────────────────
# Add your guardrail call between state.start() and smolagent_agent.run_agent().
...
# add a new /api/agent/pipeline route.  See the PLACEHOLDER below.

def _agent_thread() -> None:
    """Background thread: runs the smolagents agent and streams output to AgentState."""
    state.start()
    ...
    try:
        with log_path.open("w", encoding="utf-8") as log_fh:
            ...
            # Hand off to smolagent_agent.py — blocks until agent finishes.
...
#   2. Stage 1: run smolagent_agent.run_agent() with a fast/cheap model into a
#               temporary directory (use tempfile.TemporaryDirectory)
#   3. Stage 2: run smolagent_agent.run_agent() with the selected model, reading
...
#
# COMPARISON with ADK:
#   The _pipeline_thread() implementation is IDENTICAL for both projects.
#   Replace adk_agent.run_agent() with smolagent_agent.run_agent() — that's it.
# ──────────────────────────────────────────────────────────────────────────────
...
# restart, new models appear automatically — no UI changes needed.

@app.route("/api/model", methods=["GET"])
def api_get_model():
    available = smolagent_agent.get_available_models()
    return jsonify({
        "model":   state.model,
        "models":  available,
...
def api_set_model():
    ...
    allowed  = [m["id"] for m in smolagent_agent.get_available_models()]
...
# the browser polling repeatedly.
#
...
#
# COMPARISON with ADK:
#   This SSE implementation is IDENTICAL to the ADK version.
#   The log format ([tool_use], [result], [assistant]) is the same too —
#   _format_step() in smolagent_agent.py uses the same prefixes as
#   _format_event() in adk_agent.py.
