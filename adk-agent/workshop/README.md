# Workshop: Engineering an AI Agent with Google ADK

Welcome!  In this workshop you will explore, extend, and customise the ADK Agent
to understand how production file-processing AI agents are built and controlled
using **Google's Agent Development Kit (ADK)** for Python.
...
1. Explain the **ADK agentic loop** and how it differs from a single LLM API call
2. Identify the three types of ADK `Event` (text, tool call, tool response) in the log stream
3. Write `instruction.md` files that produce reliable, deterministic agent behaviour
4. Add new **Python tool functions** to extend what the agent can do
...
7. Chain multiple ADK agent calls into a **multi-stage processing pipeline**
...
| Exercise | Topic |
|----------|-------|
| [01 — Explore](beginner/01_explore.md) | Run the agent, trace execution, understand ADK events |
...
| [03 — Add a Python Tool](beginner/03_add_tool.md) | Write a new tool function in `adk_agent.py` |
...
| File | Role |
|------|------|
| `adk_agent.py` | Agent logic — model registry, tool definitions, ADK runner |
...
| Browser terminal | Live ADK event stream — what the agent is doing right now |
...
- Check [Google ADK docs](https://google.github.io/adk-docs) for API reference.
- Look for `# WORKSHOP` comments in `adk_agent.py` and `web_app.py` — each
