#!/usr/bin/env python3
"""
Deep Agent — Web Application
================================
Flask web UI that drives the Deep Agent harness.
All agent logic lives in agent_harness.py; this module handles HTTP,
file management, and real-time log streaming only.
...
def _agent_thread(model_name: str) -> None:
    """Background thread: delegates to agent_harness.run_agent and manages state."""
    state.start()
    state.add_log("=== Agent run started ===")

    try:
        run_agent(INBOX_DIR, OUTBOX_DIR, model_name, log_callback=state.add_log)
...
def api_list_models():
    """Return available models from configured API providers (cached 5 min)."""
    ...
    if not models:
        models = [
            {"id": "anthropic:claude-sonnet-4-6",       "display": "Claude Sonnet 4.6",       "provider": "anthropic"},
            {"id": "anthropic:claude-opus-4-7",          "display": "Claude Opus 4.7",          "provider": "anthropic"},
            {"id": "anthropic:claude-haiku-4-5-20251001","display": "Claude Haiku 4.5",         "provider": "anthropic"},
        ]

    result = {"models": models, "errors": errors}
    _models_cache = {"data": result, "expires": now + _MODELS_TTL}
